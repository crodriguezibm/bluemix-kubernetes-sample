...
- ```bx_login.sh``` - Authenticates the user to Bluemix and the Container Service. It relies on the following environment variables:
  ...
  - **BLUEMIX_ORG** - Specify your Bluemix organization when building with Travis. Not required when building with DevOps 
  - **BLUEMIX_API_URL** - Specify your Bluemix API region url. Not required when building with DevOps 
 Services pipeline.
