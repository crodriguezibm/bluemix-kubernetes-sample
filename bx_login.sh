...
if [ -z "$BLUEMIX_API_KEY" ]; then
  bx login -a "$CF_TARGET_URL" -u "$BLUEMIX_USER" -p "$BLUEMIX_PASSWORD" -c "$BLUEMIX_ACCOUNT" -o "$CF_ORG" -s "$CF_SPACE" -a https://api.ng.bluemix.net
else
  bx login --apikey "$BLUEMIX_API_KEY" -o "$CF_ORG" -s "$CF_SPACE" -a https://api.ng.bluemix.net
fi
if [ $? -ne 0 ]; then
